...
class FoodSupplementPage extends StatelessWidget {
  final String outletId;

  const FoodSupplementPage({Key? key, required this.outletId}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Suplemen Makanan', 
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Container(
                  ...
                  child: ClipRRect(
                    ...
                    child: Image.asset(
                      "assets/images/supplement_bg.png",
                      fit: BoxFit.cover, 
                      errorBuilder: (c, e, s) => Container(
                        color: Colors.redAccent.withOpacity(0.1),
                        child: const Icon(Icons.health_and_safety, size: 50, color: Colors.redAccent),
                      ),
                    ),
                  ),
                ),
                ...
                const Positioned(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text("Kesihatan dan Kecantikan", style: TextStyle(color: Colors.white70, fontSize: 14)),
                      Text(
                        "Suplemen Makanan",
            ...
            const Padding(
              ...
              child: Text("All Supplements", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            ),

            StreamBuilder<QuerySnapshot>(
              stream: FirebaseFirestore.instance
                  .collection('products')
                  .where('category', isEqualTo: 'Food Supplements')
                ...
                if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                  return Center(
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          Icon(Icons.medication_liquid, size: 70, color: Colors.grey.shade300),
                          const SizedBox(height: 16),
                          const Text("No supplements found here.", style: TextStyle(color: Colors.grey)),
                        ],
                      ),
                    ),
                  );
      ...
      onTap: () {
        Navigator.push(context, MaterialPageRoute(
          builder: (context) => ProductDetailPage(
            id: docId, // 🎯 FIX: Added ID
            name: item["name"] ?? "Supplement", 
            price: price.toString(), 
            image: imagePath, 
            outletId: outletId,
            stock: parsedStock, // 🎯 FIX: Added Stock
          ),
        ));
      },
      child: Container(
  ...
  Widget _buildProductImage(String path) {
    if (path.isEmpty) return const Icon(Icons.medical_services, size: 40, color: Colors.redAccent);
      ...
      child: Builder(
        builder: (context) {
          try {
            ...
            // Native lightweight engine reads admin base64 data payloads cleanly
